...
async def activate_profile(
    ...
    await get_raise_new(code)
    update_data = AccountUpdate(was_paying=True)
    account, code, indexes = await crud_account.update_account(
        db=session, id=profile.account_id, update_data=update_data)
    await get_raise_new(code)
    return SingleEntityResponse(data=getting_profile(obj=obj))
