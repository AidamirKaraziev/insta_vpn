...
class CrudVpnType(CRUDBase[VpnType, VpnTypeCreate, VpnTypeUpdate]):
    obj_name = "VpnType"
    ...
    async def get_all_vpn_types(self, *, db: AsyncSession, skip: int, limit: int):
        objects = await super().get_multi(db_session=db, skip=skip, limit=limit)
        return objects, 0, None

    async def get_vpn_type_by_id(self, *, db: AsyncSession, id: int):
        obj = await super().get(db=db, id=id)
        if obj is None:
            return None, self.not_found_id, None
        return obj, 0, None

    async def add_vpn_type(self, *, db: AsyncSession, new_data: VpnTypeCreate):
        """
            Проверяем id, если такой есть - возвращает ошибку.
            Проверяем name, если такой есть - возвращает ошибку.
        """
        # check id
        obj, code, indexes = await self.get_vpn_type_by_id(db=db, id=new_data.id)
        if obj:
    ...
    async def update_vpn_type(self, *, db: AsyncSession, update_data: VpnTypeUpdate, id: int):
        """
            ...
            Проверяем name, если такой есть - возвращает ошибку.
        """
        this_obj, code, indexes = await self.get_vpn_type_by_id(db=db, id=id)
        if code != 0:
            return None, code, None
        if update_data.id is not None and this_obj.id != update_data.id:
            obj, code, indexes = await self.get_vpn_type_by_id(db=db, id=update_data.id)
            if obj is not None:
                return None, self.id_is_exist, None
        # check name
...
crud_vpn_type = CrudVpnType(VpnType)
